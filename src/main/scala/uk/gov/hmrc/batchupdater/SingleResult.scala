...
object SingleResult {
  ...
  case class AlreadyUpdated(auditDetails: Map[String, String]) extends SingleResult {
    val failureReason = Some("alreadyUpdated")
  }
  case class InvalidState(auditDetails: Map[String, String]) extends SingleResult {
    val failureReason = Some("invalidState")
  }
  case class NotFound(auditDetails: Map[String, String]) extends SingleResult { val failureReason = Some("notFound") }
  case class UpdateFailed(auditDetails: Map[String, String]) extends SingleResult {
    val failureReason = Some("updateFailed")
  }
